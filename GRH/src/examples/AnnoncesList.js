...
const AnnoncesList = () => {
  ...
  const fetchAnnonces = async () => {
    ...
    try {
      ...
      const params = {
        ...
        search: filters.searchText.toLowerCase().trim() || undefined,
        metier: filters.metier || undefined,
        localisation: filters.localisation || undefined,
        typeContrat: validContractTypes.includes(filters.typeContrat) ? filters.typeContrat : undefined,
        estValide: true,
      };

      const response = await axios.get('http://localhost:5000/api/annonces/recherche', {
        params,
      ...
      setEntretienStatus(entretienResults.reduce((acc, result) => ({
        ...acc,
        [result.id]: {
      ...
      if (fetchedAnnonces.length === 0) {
        setSnackbar({
          open: true,
          message: 'Aucune annonce validée trouvée pour les critères sélectionnés.',
          severity: 'info',
        });
      }
    } catch (error) {
      setSnackbar({
        open: true,
        message: 'Erreur lors du chargement des annonces validées.',
        severity: 'error',
      });
      setAnnonces([]);
    } finally {
  ...
  const fetchSavedAnnonces = async () => {
    try {
      ...
      setSavedAnnonces((response.data.annonces || response.data).map((annonce) => annonce._id));
    } catch (error) {
      setSnackbar({
        open: true,
  ...
  const saveAnnonce = async (annonceId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(
      ...
      if (savedAnnonces.includes(annonceId)) {
        ...
        setSavedAnnonces([...savedAnnonces, annonceId]);
        setSnackbar({
          open: true,
          message: 'Annonce sauvegardée dans vos favoris !',
          severity: 'success',
        });
      }
    } catch (error) {
  ...
  const scheduleInterview = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        {
          candidat_id: selectedAnnonce.candidat?._id,
          annonce_id: selectedAnnonce._id,
    ...
    <DashboardLayout>
        ...
        {loading ? (
          ...
          <Box sx={{ textAlign: 'center', py: 6, bgcolor: '#ffffff', borderRadius: '12px', boxShadow: 2 }}>
            <Typography variant="h5" sx={{ mb: 2, color: '#1a202c' }}>
              Aucune annonce validée trouvée
            </Typography>
            <Typography variant="body1" sx={{ color: '#718096' }}>
              Modifiez vos filtres pour découvrir plus d'opportunités validées.
            </Typography>
          </Box>
        ) : (
          <Grid container spacing={3}>
                ...
                >
                  <JobCard>
                    ...
                    <CardActions sx={{ px: 2, pb: 2, bgcolor: '#fafafa' }}>
                      ...
                      <IconButton onClick={() => saveAnnonce(annonce._id)}>
      ...
      >
        ...
        <DialogActions sx={{ borderTop: '1px solid #e2e8f0', p: 2, bgcolor: '#fafafa' }}>
          ...
          >
            {entretienStatus[selectedAnnonceDetails?._id]?.planned ? 'Joindre' : 'Contacter'}
          </Button>
        </DialogActions>
      </Dialog>
