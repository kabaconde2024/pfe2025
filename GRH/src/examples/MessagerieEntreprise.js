...
const MessagerieEntreprise = () => {
  ...
  const markMessagesAsRead = useCallback(async () => {
    try {
      ...
      await axios.post(
        "http://localhost:5000/api/messages/mark-read/entreprise",
        {},
  ...
  useEffect(() => {
    const fetchAdminId = async () => {
      try {
        ...
        setErrorMessage("Erreur lors du chargement de l'administrateur.");
        setOpenSnackbar(true);
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      if (!adminId || !isPollingActive) return;
      ...
      try {
        ...
        const response = await axios.get(
          `http://localhost:5000/api/messages/admin?adminId=${adminId}&since=${since}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );

        console.log("Fetched messages from polling:", response.data);
  ...
  }, [adminId, navigate, markMessagesAsRead, userId]);
  ...
  const handleSendMessage = useCallback(
    async (e) => {
      ...
      try {
        ...
        console.log("Sending message:", { destinataireId: adminId, sujet: "Message rapide", contenu: currentMessage });
        ...
        setOpenSnackbar(true);
        // Do not add message here; let polling handle it
      } catch (error) {
    ...
    [newMessage, adminId, userId, scrollToBottom]
  );
  ...
  const messagesList = useMemo(() => {
      ...
      return (
        ...
        >
          ...
          <Typography variant="body1">
            Envoyez votre premier message pour commencer la conversation avec votre administrateur.
          </Typography>
        </Box>
      );
    }
    ...
    return messages.map((message, index) => {
      ...
      const senderName = isSender
        ? "Vous"
        : message.expediteur?.nom || message.expediteur?.nomEntreprise || "Administrateur";
        ...
        >
          ...
          >
            {(isFirstInGroup || isSender) && (
              <Typography
                variant="caption"
                sx={{ color: "text.secondary", mb: 0.5, mx: 1 }}
              >
                {senderName}
              </Typography>
            )}
                    ...
                    >
                      <Box sx={{ display: "flex", alignItems: "center" }}>
                        {message.statut === "envoyé" ? (
                          ...
                          <ReadIcon sx={{ fontSize: "1rem", color: "error.main" }} />
                        )}
                      </Box>
                    </Tooltip>
                  ) : (
