...
const CoachFormations = () => {
  ...
  const fetchFormations = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost:5000/api/formation/mes-formations-coach', {
  ...
  const handleStatusChange = async (formationId, newStatus) => {
    try {
      setStatusUpdating(true);
      await axios.patch(
        `http://localhost:5000/api/formation/${formationId}`,
        { statut: newStatus },
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );
      setFormations((prev) =>
        prev.map((f) =>
          f._id === formationId ? { ...f, statut: newStatus } : f
        )
      );
      setSelectedFormation((prev) =>
        prev && prev._id === formationId ? { ...prev, statut: newStatus } : prev
      );
      setError(null);
    } catch (err) {
  ...
  const isSessionImminent = (formation) => {
    if (!formation.horaire) return false;
    const now = new Date();
    let startTime;
    if (['virtuel', 'hybride'].includes(formation.modalite) && formation.meetLink) {
      startTime = formation.horaire.date ? new Date(formation.horaire.date) : null;
    } else if (formation.modalite === 'presentiel') {
      startTime = formation.horaire.debut ? new Date(formation.horaire.debut) : null;
    }
    if (!startTime) return false;
    const timeDiff = (startTime - now) / (1000 * 60); // Différence en minutes
    return timeDiff >= 0 && timeDiff <= 30; // Session dans les 30 prochaines minutes
  };
  ...
  const renderProgressBar = (contenus) => {
    const total = contenus.length;
    const completed = Math.floor(Math.random() * (total + 1)); // Exemple aléatoire
    const progress = total > 0 ? (completed / total) * 100 : 0;
    return (
      <Box sx={{ mt: 2 }}>
        <Typography variant="caption">Progression de l'employé</Typography>
      ...
      <>
        ...
        >
          ...
          <Typography variant="body1" sx={{ color: 'text.secondary' }}>
            Supervisez les formations qui vous sont assignées en tant que coach
          </Typography>
        </Box>
              ...
              >
                <CardContent sx={{ flexGrow: 1 }}>
                  <Box display="flex" justifyContent="space-between" alignItems="center">
                    ...
                    {['virtuel', 'hybride'].includes(formation.modalite) && isSessionImminent(formation) && (
                      <Badge badgeContent="Bientôt" color="warning">
                  ...
                  <Grid container spacing={1} sx={{ mt: 1 }}>
                    ...
                    {['virtuel', 'hybride'].includes(formation.modalite) && formation.meetLink && (
                      <Grid item xs={12}>
                        <Typography variant="caption" display="block" color="text.secondary">
                          Date
                        </Typography>
                    ...
                    {formation.modalite === 'contenu' && formation.contenus && (
                      <Grid item xs={12}>
                        {renderProgressBar(formation.contenus)}
                      </Grid>
                    )}
                  </Grid>
                </CardContent>
                ...
                >
                  {['virtuel', 'hybride'].includes(formation.modalite) && formation.meetLink && (
                    <Button
                      ...
                      href={formation.meetLink}
                      target="_blank"
                      rel="noopener noreferrer"
                      disabled={formation.statut !== 'planifie' && formation.statut !== 'en-cours'}
                    >
                      Rejoindre Meet
                    </Button>
                  )}
                ...
                <Card sx={{ mb: 2 }}>
                  <CardContent>
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12} sm={6}>
                        ...
                        <Box display="flex" alignItems="center" gap={1}>
                          ...
                          <FormControl variant="outlined" size="small" sx={{ minWidth: 120 }}>
                            ...
                            <Select
                              value={selectedFormation.statut}
                              onChange={(e) => handleStatusChange(selectedFormation._id, e.target.value)}
                      ...
                      {['virtuel', 'hybride'].includes(selectedFormation.modalite) && selectedFormation.meetLink && (
                        <Grid item xs={12} sm={6}>
                          ...
                          <Typography variant="body2">
                            {selectedFormation.meetLink ? (
                              <Button
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                disabled={selectedFormation.statut !== 'planifie' && selectedFormation.statut !== 'en-cours'}
                              >
                                Rejoindre Meet
                              </Button>
                            ) : (
                      ...
                      <Grid item xs={12} sm={6}>
                        ...
                        <Typography variant="body2">
                          {selectedFormation?.employee?.nom || '-'} (
                          {selectedFormation?.employee?.userId || '-'}
                          )
                        </Typography>
                      </Grid>
                      <Grid item xs={12} sm={6}>
                        ...
                        <Grid item xs={12}>
                          ...
                          <TableContainer sx={{ maxHeight: 300, overflow: 'auto' }}>
                            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                              ...
                              <tbody>
                                {(selectedFormation.contenus || []).map((contenu, index) => (
                          ...
                          {selectedFormation.modalite === 'contenu' && renderProgressBar(selectedFormation.contenus)}
                        </Grid>
                      )}
                    </Grid>
                  </CardContent>
